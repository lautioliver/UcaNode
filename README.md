...
UcaNode sigue el patrón **Server Components + Server Actions** de Next.js:

1. Las **páginas** (`src/app/**/page.tsx`) son Server Components: leen datos directamente con Prisma en el servidor.
2. Los **formularios HTML** envían datos a **Server Actions** (`src/lib/actions.ts`), que escriben en la base y revalidan la caché.
3. Los **componentes** (`src/components/`) solo reciben props ya resueltas; no acceden a la base de datos.
4. **`src/lib/labels.ts`** centraliza las etiquetas y colores de los enums de Prisma.
5. **`prisma/schema.prisma`** define el modelo; el cliente generado vive en `src/generated/prisma/`.
...
flowchart TB
    ...
    UI --> Pages
    Forms -->|"action={createX}"| Actions
    Pages -->|"await prisma.*"| PrismaLib
    Actions -->|"await prisma.*"| PrismaLib
    PrismaLib --> DB
    Schema --> PrismaLib
    Seed --> DB
```
...
flowchart LR
    ...
    Materias --> MateriaCard & Actions
    MateriaId --> Calendario
    Entregas --> Calendario & Actions
    Horarios --> LayoutComp & Actions
    Links --> LayoutComp & Actions
    Perfil --> LayoutComp & Actions
...
| `src/lib/actions.ts` | Server Actions: `createMateria`, `createEntrega`, `createHorario`, `createLink`, `updatePerfil` |
...
sequenceDiagram
    ...
    F->>A: createMateria(formData)
    A->>Pr: prisma.materia.create(...)
    Pr->>DB: INSERT
    DB-->>Pr: OK
    A->>C: revalidatePath("/materias")
    A-->>F: redirect / refresh
    F-->>U: Página actualizada
```

Cada acción de escritura llama a `revalidatePath()` en las rutas afectadas para que Next.js regenere las páginas con datos frescos.
